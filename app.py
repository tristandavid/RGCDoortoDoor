...
def _get_fcm_access_token():
    """Return a short-lived OAuth2 bearer token for the FCM HTTP v1 API.
    Returns None if the service account file isn't present."""
    if not os.path.exists(FCM_SERVICE_ACCOUNT_FILE):
        return None
    try:
        creds = service_account.Credentials.from_service_account_file(
            FCM_SERVICE_ACCOUNT_FILE, scopes=_FCM_SCOPES,
        )
        creds.refresh(google.auth.transport.requests.Request())
        return creds.token
    except Exception as exc:
        app.logger.error("FCM auth failed: %s", exc)
        return None

def _send_fcm_push(fcm_token, type_, title, body):
    """Send a data push to one device via FCM HTTP v1 API.
    ...
    payload = json.dumps({
        "message": {
            ...
            # Android: deliver even when the device is in Doze mode.
    ...
    try:
        ...
        app.logger.error("FCM push failed (%s): %s", exc.code, exc.read())
        return False
    except Exception as exc:
        app.logger.error("FCM push error: %s", exc)
        return False

def _push_to_all_devices(type_, title, body):
...
class FcmToken(db.Model):
    """One FCM device token per Android app install. The companion app
    registers its token on every login and whenever FCM refreshes it
    (see /api/v1/fcm/register). Upserted by token value so re-installs
    do not accumulate stale rows."""
    ...
    if request.method == "POST":
        ...
        if not email:
            ...
            _push_to_all_devices(
                "mailbox",
                "New message",
                f"New message from {name or email} via the contact form.",
            )
...
def book_a_pickup():
    if request.method == "POST":
        ...
        _push_to_all_devices(
            "pickup",
            "New pickup request",
            f"Pickup request from {pickup.name} on {pickup.pickup_date.strftime('%b %d')}.",
        )
...
@app.route("/api/v1/fcm/register", methods=["POST"])
@api_login_required
def api_fcm_register():
    """Store or refresh the caller's FCM device token. The Android app calls
    this on every login and whenever FCM issues a new token. We upsert by
    token value so re-installs do not accumulate stale rows."""
